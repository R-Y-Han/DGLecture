 ...
 * @brief Ultra Weak DG scheme for u_t + u_{xxx} = 0
 * @version 0.1
 * @date 2022-05-26
 * 
...
const int k = 2;    /**< polynomial order of the basis */
...
double** L(double **ut)
{
    ...
    for (j=1; j<=n; j++)
    {
        for (m=0; m<=k; m++)
        {
            ...
            ans[j-1][m] = ans[j-1][m] - ul * phixx(m,1) * 4 / (h * h);
            ...
            for (l=0; l<=k; l++)
            {
                uxr = uxr + ut[temp][l] * phix(l,-1) * 2 / h;
            }
            ans[j-1][m] = ans[j-1][m] + uxr * phix(m,1) * 2 / h;

            uxr = 0;
            for (l=0; l<=k; l++)
            {
                uxr = uxr + ut[j-1][l] * phix(l,-1) * 2 / h;
            }
            ans[j-1][m] = ans[j-1][m] -uxr * phix(m,-1) * 2 / h;
            ...
            for (l=0; l<=k; l++)
            {
                uxxr = uxxr + ut[temp][l] * phixx(l,-1) * 4 / (h*h);
            }
            ans[j-1][m] = ans[j-1][m] - uxxr * phi(m,1);

            uxxr=0;
            for (l=0; l<=k; l++)
            {
                uxxr = uxxr + ut[j-1][l] * phixx(l,-1) * 4 / (h*h);
            }
            ans[j-1][m] = ans[j-1][m] + uxxr * phi(m,-1);
